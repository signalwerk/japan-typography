// https://paged.signalwerk.workers.dev/?originHostname=typography.japan.signalwerk.ch&addBodyClass=pagedjs--pdf

const COOKIE_NAME = "OriginHostname";
const GET_NAME = "originHostname";
const BODY_CLASS_NAME = "addBodyClass";
...
export default {
  async fetch(request, env, ctx) {
    ...
    // base64 of https://gitlab.coko.foundation/pagedjs/interface-polyfill/-/blob/master/interface.css?ref_type=heads
...
<!-- disable force reflow - version 0.5.0-beta.0 looks good -->
<!--
<script>
...
`,
    ...
    class BodyElementHandler {
      element(element) {
        ...
        const origClass = element.getAttribute("class");
        const adddClass =
          url.searchParams.get(BODY_CLASS_NAME) || "pagedjs--server-side";
        if (origClass) {
          element.setAttribute("class", `${origClass} ${adddClass}`);
        } else {
          element.setAttribute("class", adddClass);
        }
      }
    }
    ...
    if (contentType && contentType.startsWith("text/html")) {
      return new HTMLRewriter()
        .on("head", new HeaderElementHandler())
        .on("body", new BodyElementHandler())
